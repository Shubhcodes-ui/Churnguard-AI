...
export default function LoginPage() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      setError('root', { message: err.response?.data?.detail || 'Invalid email or password' });
    }
  };
    ...
    <div className="min-h-screen bg-cg-base flex items-center justify-center p-4">
      ...
      >
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-extrabold text-cg-primary tracking-tight">ChurnGuard AI</h1>
          <p className="text-xs text-cg-muted">Sign in to your risk analytics workspace</p>
        </div>

        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-cg-muted uppercase tracking-wider mb-1.5">Email Address</label>
            <input
              {...register('email')}
              type="email"
              ...
              placeholder="analyst@churnguard.ai"
            />
        ...
        <p className="text-center text-cg-muted text-xs pt-2">
          ...
          <Link to="/signup" className="text-cg-brand hover:underline font-semibold">
            Create account
          </Link>
        </p>
      </motion.div>
